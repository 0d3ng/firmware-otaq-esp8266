...
static void IRAM_ATTR gpio_DHT_isr_handler(void *arg)
{
	// record time
	if (isr_count < 64)
		edge_times[isr_count] = esp_timer_get_time();
	// count pulses
	isr_count++;
...
int DHTget(uint16_t *temperature, uint16_t *humidity)
{
	...
	*humidity = 0;
	...
	if (_VERBOSE_)
		ESP_LOGI(TAG, "count %d", isr_count);
	// check we got all the pulses
	if (isr_count >= 42)
	{
		if (isr_count != 42)
			ESP_LOGE(TAG, "WARNING pulse count %d != 42", isr_count);
		...
		//		for (unsigned int i=1; i<41; i++)
		{
			...
			if (pulse_width_us > EDGE_THRESHOLD_MICROSECONDS)
				data[j] = data[j] | mask;
		...
		if (_DEBUG_)
			for (int k = 0; k < 5; k++)
			{
				printf("%d: ", k);
				// printbinary(data[k]);
				for (uint8_t maskb = 0x80; maskb; maskb = maskb >> 1)
					printf((data[k] & maskb) ? "1" : "0");
				printf("\n");
			}
	...
	{
		if (_VERBOSE_)
			ESP_LOGE(TAG, "Wrong pulse count %d != 41", isr_count);
		return -1;
	}
	fflush(stdout);
	return 0;
